...
for i=1:n
    for j=1:n
        sij = s(i,j);
        sig = delta(sij-s(pbc(i+1,n),j)) + delta(sij-s(i,pbc(j+1,n))) + delta(sij-s(pbc(i-1,n),j)) + delta(sij-s(i,pbc(j-1,n)));
...
% flip the spin and calculate energy change
    ...
    sign = delta(sij-s(pbc(i+1,n),j)) + delta(sij-s(i,pbc(j+1,n))) + delta(sij-s(pbc(i-1,n),j)) + delta(sij-s(i,pbc(j-1,n)));
    sigo = delta(sijo-s(pbc(i+1,n),j)) + delta(sijo-s(i,pbc(j+1,n))) + delta(sijo-s(pbc(i-1,n),j)) + delta(sijo-s(i,pbc(j-1,n)));
    del = -(sign - sigo);
